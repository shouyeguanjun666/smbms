...
<div class="right">
        ...
        <div class="providerAdd">
          <form id="billForm" name="billForm" method="post" action="${pageContext.request.contextPath }/bill/doupbill">
                ...
                <div>
					...
					<select name="providerId" id="providerId">
		        		<c:forEach items="${providerList}" var="provider">
		        			<option  <c:if test="${provider.id==bill.providerId}">selected="selected"</c:if> value="${provider.id}">${provider.proName}</option>
		        		</c:forEach>
		        	</select>
					<font color="red"></font>
                </div>
