...
function deleteBill(obj){
	...
	$.post(path+"/bill/delBill","id="+obj.attr("billid"),function (data) {
			alert(data.result);
			if(data.result=="true"){
				cancleBtn();
				obj.parents("tr").remove();
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/bill/billview?id="+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/bill/gobillup?id="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
